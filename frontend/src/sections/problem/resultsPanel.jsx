...
/* =====================================================
   MAIN COMPONENT
===================================================== */
export default function ResultsPanel({ runResult, loading, error }) {
  const [activeTab, setActiveTab] = useState(0);

  /* =====================================================
        BACKEND NON-SUBMISSION ERROR
  ====================================================== */
  const backendError =
    error &&
    runResult &&
    (!Array.isArray(runResult.data) || runResult.data.length === 0);
  ...
  const testCases =
    runResult && Array.isArray(runResult.data) ? runResult.data : [];
  ...
  const allPassed =
    ...
    testCases.every(
      (t) =>
        String(t.output).trim() === String(t.expected_output).trim()
    );

  /* =====================================================
        AUTO TOAST FOR BACKEND ERRORS
  ====================================================== */
  useEffect(() => {
    if (backendError) {
  ...
  return (
    <div className={`flex flex-col h-full w-full ${COLORS.bg}`}>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            {loading ? (
              ...
              <AlertTriangle size={18} className={COLORS.error} />
            ) : !runResult ? (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#111] relative">
        ...
        {/* EMPTY */}
        {!runResult && !loading && !backendError && (
        ...
        {backendError && (
          <div className="p-6">
            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 font-mono text-red-200 whitespace-pre-wrap">
              <div className="flex items-center gap-2 mb-2 border-b border-red-500/20 pb-2 text-red-400 font-semibold">
        ...
        {runResult && (isRuntimeError || isTLE) && (
          <div className="p-6">
            <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-4 font-mono text-red-200 whitespace-pre-wrap">
        ...
        {runResult && isCompilationIssue && !isRuntimeError && !isTLE && (
          <div className="p-6">
            <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-4 font-mono text-amber-200 whitespace-pre-wrap">
              ...
              {testCases.map((t, i) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded text-xs font-medium transition-colors border flex items-center gap-2 ${
                      activeTab === i
